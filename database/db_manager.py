...
class DatabaseManager:
    ...
    def save_data(self, table_name: str, data: Dict[str, Any],
        ...
        try:
            ...
            # 检查记录是否存在
            if self._check_existing(table_name, key_field, data.get(key_field)):
                return self._update_if_needed(table_name, filtered_data, key_field)
    ...
    def batch_save(self, table_name: str, data_list: List[Dict[str, Any]],
        ...
        for data in data_list:
            try:
                # 使用队列管理批量操作 (保留性能优化点)
                self.batch_queue.put((table_name, data, key_field))

                # 实际处理
                if self.save_data(table_name, data, key_field):
                    if self._check_existing(table_name, key_field, data.get(key_field)):
                        batch_stats['update'] += 1
                    else:
                        batch_stats['insert'] += 1
                else:
                    batch_stats['error'] += 1
        ...
        with self.lock:
            for k in batch_stats:
                if k != 'total':
                    self.stats[k] += batch_stats[k]

        return batch_stats

    def _check_existing(self, table_name: str, key_field: str, key_value: Any) -> bool:
        """检查记录是否存在"""
        sql = f"SELECT 1 FROM `{table_name}` WHERE `{key_field}` = %s LIMIT 1"
        result = db_config.execute_sql(sql, (key_value,), fetch_one=True)
        return bool(result)
